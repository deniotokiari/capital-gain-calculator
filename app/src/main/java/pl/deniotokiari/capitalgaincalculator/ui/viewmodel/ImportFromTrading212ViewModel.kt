...
import pl.deniotokiari.capitalgaincalculator.domain.usecase.AddImportedValuesUseCase
...
class ImportFromTrading212ViewModel(
    ...
    private val addImportedValuesUseCase: AddImportedValuesUseCase,
    private val appHostNavigation: AppHostNavigation
) : ViewModel() {
    private val _uiState = MutableStateFlow(ImportFromRevolutViewModel.UiState(loading = false))
    val uiState: StateFlow<ImportFromRevolutViewModel.UiState> = _uiState
    ...
    fun onDocumentSelected(uri: Uri?) {
        viewModelScope.launch {
            ...
            if (map == null) {
                ...
                addImportedValuesUseCase(
                    AddImportedValuesUseCase.Params(
                        map = map,
                        portfolioId = id
                    )
                )
